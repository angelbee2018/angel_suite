...
mc_map2 <- function(
  ...
  if (is.null(.job_name)) {
    .job_name <- paste("mc_map2_", .epoch_time, sep = "")
  }
  ...
  # check if all the list elements are of equal length
  .list_length <- unique(unlist(lapply(X = list(.x, .y), FUN = function(a1) {return(length(a1))} )))
  ...
  # define function to be fun
  function_to_run <- function(.x_current, .y_current, .f, .globals_save_dir, .intermediate_files_dir, .job_name, .progress, .debug, ..i, .status_messages_dir_stdout, .status_messages_dir_stderr) {
    ...
    if (flag_completion == TRUE) {
      ...
      if (number_of_processes_alive < .no_workers & current_map_index < map_length) {
        ...
        for (..i in (new_map_start):min(c(new_map_end, map_length))) {
          ...
          # define chunk for the target of mapping operation
          .x_current <- .x[list_start_end_indices$start:list_start_end_indices$end]
          .y_current <- .y[list_start_end_indices$start:list_start_end_indices$end]
          
          .status_messages_dir_stdout <- paste(.status_messages_dir, "/chunk_", ..i, "_stdout.txt", sep = "")
          .status_messages_dir_stderr <- paste(.status_messages_dir, "/chunk_", ..i, "_stderr.txt", sep = "")
          
          list_workers[[..i]] <- parallel:::mcparallel(
            expr = function_to_run(".x_current" = .x_current, ".y_current" = .y_current, ".f" = .f, ".globals_save_dir" = .globals_save_dir, ".intermediate_files_dir" = .intermediate_files_dir, ".job_name" = .job_name, ".progress" = .progress, ".debug" = .debug, "..i" = ..i, ".status_messages_dir_stdout" = .status_messages_dir_stdout, ".status_messages_dir_stderr" = .status_messages_dir_stderr), 
            detached = TRUE
          )
...
mc_map <- function(
  ...
  if (is.null(.job_name)) {
    .job_name <- paste("mc_map_", .epoch_time, sep = "")
  }
  ...
  # .l_chunked <- lapply(X = 1:.no_chunks, FUN = function(a1) {return(lapply(X = .x, FUN = function(b1) {return(b1[parallel::splitIndices(nx = .list_length, ncl = .no_chunks)[[a1]]])} ))} )
  
  # END CHUNKING ###
  
  # define function to be fun
  function_to_run <- function(.x_current, .f, .globals_save_dir, .intermediate_files_dir, .job_name, .progress, .debug, ..i, .status_messages_dir_stdout, .status_messages_dir_stderr) {
    ...
    if (flag_completion == TRUE) {
      ...
      if (number_of_processes_alive < .no_workers & current_map_index < map_length) {
        ...
        for (..i in (new_map_start):min(c(new_map_end, map_length))) {
          ...
          list_workers[[..i]] <- parallel:::mcparallel(
            expr = function_to_run(".x_current" = .x_current, ".f" = .f, ".globals_save_dir" = .globals_save_dir, ".intermediate_files_dir" = .intermediate_files_dir, ".job_name" = .job_name, ".progress" = .progress, ".debug" = .debug, "..i" = ..i, ".status_messages_dir_stdout" = .status_messages_dir_stdout, ".status_messages_dir_stderr" = .status_messages_dir_stderr), 
            detached = TRUE
          )
